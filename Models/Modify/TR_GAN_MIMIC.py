...
def train_batch(hidden_size, learning_rate, l2_regularization, save_epoch):
    ...
    while train_set.epoch_completed < epochs:
        with tf.GradientTape() as agent_tape, tf.GradientTape() as tape, tf.GradientTape() as dis_tape:
            ...
            _, discriminator_prob_line = discriminator([states, rewards, actions_values])
            offline_rewards_labels_calculate = reward_calculate(offline_rewards_labels, predicted_visit)
            _, discriminator_prob_offline = discriminator([offline_states_labels[:, :-1, :], offline_rewards_labels_calculate, offline_actions_values_label])
            ...
            # m_x和m_r的损失函数,其中online采用鉴别器更新，offline采用mse等真实数据更新
            gen_loss_online = cross_entropy(tf.ones_like(discriminator_prob_line), discriminator_prob_line)
            ...
            gen_loss_online = gen_loss_online + gen_loss_offline + gen_loss_offline_

            # m_d的损失函数
            d_loss_online = cross_entropy(tf.zeros_like(discriminator_prob_line), discriminator_prob_line)
            d_loss_offline = cross_entropy(tf.ones_like(discriminator_prob_offline), discriminator_prob_offline)
            d_loss = d_loss_online + d_loss_offline
        ...
        # 计算价值函数
        online_value = discount_reward(rewards, gamma, lambda_imbalance, discriminator_prob_line)
        offline_value = discount_reward(offline_rewards_labels_calculate, gamma, lambda_imbalance, discriminator_prob_offline)
        ...
        if train_set.epoch_completed % 1 == 0 and train_set.epoch_completed not in logged:
            ...
            test_online_representation, dis_prob_test_online = discriminator([states_test_online, rewards_test_online_calculate, actions_values_test_online])
            test_offline_representation, dis_prob_test_offline = discriminator([states_test_offline, rewards_test_offline_calculate, actions_values_test_offline])

            test_value_online = discount_reward(rewards_test_online_calculate, gamma, lambda_imbalance, dis_prob_test_online)
            test_value_offline = discount_reward(rewards_test_offline_calculate, gamma, lambda_imbalance, tf.ones_like(dis_prob_test_offline)*0.5)
            ...
            print('epoch {}  agent_loss {} '
                  ...
                  .format(train_set.epoch_completed,
                          ...
                          tf.reduce_mean(discriminator_prob_line),
                          tf.reduce_mean(discriminator_prob_offline),
                          tf.reduce_mean(dis_prob_test_online),
                          tf.reduce_mean(dis_prob_test_offline),
                          np.sum(death_estimated_online_test)))

            if tf.reduce_mean(test_value_online) > 2.57 and (tf.reduce_mean(discriminator_prob_line)-0.5) < 1e-8:
                np.save('11_30 最终版\\1_28_states_TR_GAN_mimic.npy'+ str(save_epoch)+'_'+str(train_set.epoch_completed), states_test_online)
                np.save('11_30 最终版\\1_28_death_TR_GAN_mimic.npy'+ str(save_epoch)+'_'+str(train_set.epoch_completed), death_estimated_online_test)
                np.save('11_30 最终版\\1_28_rewards_TR_GAN_mimic.npy'+ str(save_epoch)+'_'+str(train_set.epoch_completed), rewards_test_online)
                np.save('11_30 最终版\\1_28_actions_TR_GAN_mimic.npy'+ str(save_epoch)+'_'+str(train_set.epoch_completed), actions_values_test_online)
                np.save('11_30 最终版\\1_28_discount_reward_TR_GAN_mimic.npy'+ str(save_epoch)+'_'+str(train_set.epoch_completed), test_value_online)
                print('保存成功！')
...
if __name__ == '__main__':
    test_test('TR_GAN__1_7_mimic_save.txt')
    ...
    # print(TR_GAN.max)
    for i in range(10):
        test_online_value = train_batch(hidden_size=4, learning_rate=0.0002258178177978965, l2_regularization=0.1, save_epoch=i)
